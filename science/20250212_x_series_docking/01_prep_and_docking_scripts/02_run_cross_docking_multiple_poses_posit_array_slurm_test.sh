#!/bin/bash
#SBATCH --job-name=posit_multiple_poses
#SBATCH --output=logs/posit_%A_%a.out
#SBATCH --error=logs/posit_%A_%a.stderr
...
asap-cli docking cross-docking \
  ...
  --cache-dir $cache \
  --output-dir "/data1/choderaj/paynea/asap-datasets/20250212_p_to_x_posit/${SLURM_ARRAY_TASK_ID}" \
  --no-overwrite \
  --no-save-to-cache \
  --use-only-cache \
  --num-poses 50 \
